...
class BiFPN(nn.Module):
    ...
    def forward(self, p3,p4,p5,p6 = None,p7 = None):
        """
        ...
        if self.first_time:
            ...
            p3_in, p4_in, p5_in, p6_in, p7_in = p3 , p4, p5, p6, p7
        ...
        p6_up = self.conv6_up(
            self.swish(weight[0] * p6_in + weight[1] * self.matching(self.p6_upsample(p7_in), shape=p6_in.shape[-2:])))
        ...
        p5_up = self.conv5_up(
            self.swish(weight[0] * p5_in + weight[1] * self.matching(self.p5_upsample(p6_up), p5_in.shape[-2:])))
        ...
        p4_up = self.conv4_up(
            self.swish(weight[0] * p4_in + weight[1] * self.matching(self.p4_upsample(p5_up), p4_in.shape[-2:])))
        ...
        p3_out = self.conv3_up(
            self.swish(weight[0] * p3_in + weight[1] * self.matching(self.p3_upsample(p4_up), p3_in.shape[-2:])))
        ...
        p4_out = self.conv4_down(
            self.swish(weight[0] * p4_in + weight[1] * p4_up + weight[2] * self.matching(self.p4_downsample(p3_out),
                                                                                         p4_in.shape[-2:])))
        ...
        p5_out = self.conv5_down(
            self.swish(weight[0] * p5_in + weight[1] * p5_up + weight[2] * self.matching(self.p5_downsample(p4_out),
                                                                                         p5_in.shape[-2:])))
        ...
        p6_out = self.conv6_down(
            self.swish(weight[0] * p6_in + weight[1] * p6_up + weight[2] * self.matching(self.p6_downsample(p5_out),
                                                                                         p6_in.shape[-2:])))
        ...
        p7_out = self.conv7_down(
            self.swish(weight[0] * p7_in + weight[1] * self.matching(self.p7_downsample(p6_out), p7_in.shape[-2:])))
...
if __name__ == "__main__":
    testP3 = torch.ones(size=[3, 16, 129, 129])
    testP4 = torch.ones(size=[3, 32, 69, 69])
    testP5 = torch.ones(size=[3, 64, 24, 24])
    testBiFPN = BiFPN(num_channels=128,conv_channels=[16, 32,64],first_time=True)
    rP3, rP4, rP5, rP6, rP7 = testBiFPN(testP3, testP4, testP5)
    rP3, rP4, rP5, rP6, rP7 = BiFPN(num_channels=128,conv_channels=[],first_time=False)(rP3, rP4, rP5, rP6, rP7)
    print(rP3.shape)
    print(rP4.shape)
    print(rP5.shape)
    print(rP6.shape)
