...
class BiFPN(nn.Module):
    ...
    def __init__(self, num_channels, conv_channels, first_time=False, epsilon=1e-4):
        ...
        # Conv layers
        self.conv4_up = SeparableConvBlock(num_channels,activation=True)
        self.conv3_up = SeparableConvBlock(num_channels,activation=True)
        self.conv4_down = SeparableConvBlock(num_channels,activation=True)
        self.conv5_down = SeparableConvBlock(num_channels,activation=True)
        ...
        if self.first_time:
            self.p5_down_channel = nn.Sequential(
                Conv2dStaticSamePadding(conv_channels[2], num_channels, 1,bias=False),
                nn.BatchNorm2d(num_channels,eps=1e-3,momentum=0.01),
            )
            self.p4_down_channel = nn.Sequential(
                Conv2dStaticSamePadding(conv_channels[1], num_channels, 1,bias=False),
                nn.BatchNorm2d(num_channels,eps=1e-3,momentum=0.01),
            )
            self.p3_down_channel = nn.Sequential(
                Conv2dStaticSamePadding(conv_channels[0], num_channels, 1,bias=False),
                nn.BatchNorm2d(num_channels,eps=1e-3,momentum=0.01),
            )
    ...
    def forward(self, p3,p4,p5):
        ...
        p4_up = self.conv4_up(weight[0] * p4_in + weight[1] * F.interpolate(p5_in, p4_in.shape[-2:]))

        # Weights for P3_0 and P4_1 to P3_2
        p3_w1 = self.p3_w1_relu(self.p3_w1)
        weight = p3_w1 / (torch.sum(p3_w1, dim=0) + self.epsilon)
        # Connections for P3_0 and P4_1 to P3_2 respectively
        p3_out = self.conv3_up(weight[0] * p3_in + weight[1] * F.interpolate(p4_up, p3_in.shape[-2:]))
        ...
        p4_out = self.conv4_down(
            weight[0] * p4_in + weight[1] * p4_up + weight[2] * F.interpolate(self.p4_downsample(p3_out),p4_in.shape[-2:]))
        ...
        p5_out = self.conv5_down(
            weight[0] * p5_in + weight[1] * F.interpolate(self.p5_downsample(p4_out),p5_in.shape[-2:]))
